...
public class DocumentsController(IDocumentService documentService, ILogger<DocumentsController> logger, IOcrService ocrService)
    : ControllerBase
{
    ...
    public ActionResult<List<OcrDocumentDto>> GetDocumentsByStatus(int statusId, DateTime? startDate=null, DateTime? endDate=null)
    {
        try
        {
    ...
    public ActionResult<List<OcrDocumentDto>> GetDocumentDetailsById(Guid documentId)
    {
        ...
        {
            _logger.LogError($"An error has ocurred querying the documents with id {documentId}", $"{e.Message}\n{e.StackTrace}");
            return StatusCode(StatusCodes.Status500InternalServerError);
        }
    }
}